...
const TESTS_PASS_CLAIM =
  /(?<!\bnot )(?<!\bno )(?<!\bhaven'?t )(?<!\bdidn'?t )(?<!\bnever claim )(?<!\bwithout )(?<!\bthat )(?<!\bonce )(?<!\bwhen )(?<!\bif )\b(?:all tests pass(?:ed)?|(?<!\ball )tests pass(?:ed)?|npm (?:run )?test(?::fast)? pass(?:ed)?|test suite pass(?:es|ed)?|tests are passing)\b/i;
const COMMIT_CLAIM =
