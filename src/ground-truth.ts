...
const TESTS_PASS_CLAIM =
  /(?<!\bnot )(?<!\bno )(?<!\bhaven'?t )(?<!\bdidn'?t )(?<!\bnever claim )(?<!\bwithout )(?<!\bthat )\b(tests pass(?:ed)?|all tests pass|npm (?:run )?test(?::fast)? pass(?:ed)?|test suite pass(?:es|ed)?|tests are passing)\b/i;
const COMMIT_CLAIM =
