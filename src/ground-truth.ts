...
const DONE_CLAIM =
  /\b(?:all done|task (?:is )?complete|ready to merge|this is done|work is finished)(?!\s+(?:once|when|if|after|until|before)\b)|should work now(?! that)|finished(?:\.|!|$)/i;
...
const COMMIT_CLAIM =
  /(?<!\bnot )(?<!\bno )(?<!\bhaven'?t )(?<!\bdidn'?t )(?<!\bonce )(?<!\bwhen )(?<!\bif )(?<!\bafter )(?<!\buntil )(?<!\bbefore )\bcommitted\b/i;
const PUSH_CLAIM =
  /(?<!\bnot )(?<!\bno )(?<!\bhaven'?t )(?<!\bdidn'?t )(?<!\bonce )(?<!\bwhen )(?<!\bif )(?<!\bafter )(?<!\buntil )(?<!\bbefore )\b(pushed to origin|pushed)\b/i;
